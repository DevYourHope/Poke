...
export default function TeamAnalyzer() {
  ...
  const createNewTeam = () => {
    const newTeam: Team = {
      id: `team-${Date.now()}`,
      name: `Squadra ${allTeams.length + 1}`,
  ...
  const deleteTeam = (id: string) => {
    if (allTeams.length <= 1) {
      alert("Devi avere almeno una squadra!");
      return;
    }
  ...
  const generateOptimizedTeam = async (starterName: string) => {
    ...
    try {
      ...
      if (recommendedNames.length === 0) throw new Error("Nessun Pokémon consigliato.");
      ...
      alert("Errore durante la generazione della squadra. Riprova.");
    } finally {
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      if (analysisMode === 'casual') {
        prompt = `Sei un amichevole Professore Pokémon. Analizza questa squadra per il gioco "${selectedGame.name}" in modo SEMPLICISSIMO.
        Squadra: ${teamInfo}.
        
        Fornisci un'analisi breve e incoraggiante focalizzata su:
        1. "Vibe" della squadra: È un team equilibrato o divertente?
        2. Grandi Ostacoli: Contro quale Capopalestra o Superquattro bisogna stare un po' più attenti? (Solo i più importanti).
        3. Un consiglio flash: Una mossa o un Pokémon da aggiungere per stare tranquilli.
        
        Usa un linguaggio semplice, niente termini tecnici come "EVs", "IVs" o "Sweeper". Rispondi in italiano.`;
      } else {
        prompt = `Sei un analista competitivo di Pokémon (Try-harder). Analizza questa squadra per il gioco "${selectedGame.name}" in modo ESTREMAMENTE DETTAGLIATO e TECNICO.
        Squadra: ${teamInfo}.
        
        Fornisci un'analisi brutale e approfondita su:
        1. Matchup Specifici: Analizza la copertura contro OGNI Capopalestra, Superquattro e il Campione. Chi vince e chi perde?
        2. Core e Sinergie: Analizza le debolezze di tipo incrociate e se mancano ruoli fondamentali (Wall, Sweeper, Pivot).
        3. Ottimizzazione Pro: Suggerisci set di mosse specifici, strumenti tenuti e possibili sostituzioni per massimizzare il win-rate.
        
        Usa termini tecnici (STAB, Coverage, Check, Counter). Sii critico se il team ha falle evidenti. Rispondi in italiano.`;
      }
      ...
      setAnalysisResult(response.text || "Impossibile generare l'analisi al momento.");
    } catch (error) {
  ...
  const addToTeam = async (pokemon: Pokemon) => {
    if (team.length >= 6) {
      alert('La squadra è già al completo (max 6 Pokémon)!');
      return;
    }
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            {editingTeamName !== null ? (
              ...
              <h2 className="text-3xl md:text-5xl lg:text-6xl font-black uppercase italic tracking-tighter flex items-center flex-wrap gap-3">
                {currentTeam?.name || 'Team'} <span className="text-blue-500">Builder</span>
                <button 
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <span className="text-[7px] font-black text-slate-500 uppercase tracking-widest block">Squadra</span>
            ...
            <div className="relative flex-1 min-w-[160px] sm:flex-none">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <span className="text-[7px] font-black text-slate-500 uppercase tracking-widest block">Gioco</span>
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-black uppercase italic tracking-tighter">Analisi <span className="text-emerald-500">RotomDex</span></h3>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-800">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Debolezze</span>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {TYPES.filter(t => analysis.defensive[t] > 0).length === 0 && <span className="text-xs text-slate-600 italic">Nessuna</span>}
                  </div>
                </div>
                <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-800">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Resistenze</span>
                  <div className="flex flex-wrap gap-2">
                    {TYPES.filter(t => analysis.defensive[t] < 0).map(t => (
                      <div key={t} className="flex items-center gap-1">
                        <div className={`w-3 h-3 rounded-full ${TYPE_COLORS[t]}`}></div>
      ...
      <AnimatePresence>
        {isStarterModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <h3 className="text-3xl font-black uppercase italic tracking-tighter mb-2">Scegli Starter</h3>
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-8">Seleziona il tipo di starter per generare la squadra</p>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
            ...
            >
              <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 sticky top-0 z-10">
                <div>
                  <h3 className="text-3xl font-black uppercase italic tracking-tighter">Scegli Pokémon</h3>
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Aggiungi un membro alla squadra</p>
                </div>
                <button 
                  onClick={() => setIsSelectorOpen(false)}
              ...
              <div className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
                ...
                {filteredPokemon.totalPages > 1 && (
                  <div className="flex items-center justify-center gap-4 mt-8">
                    ...
                    <span className="text-sm font-black italic text-slate-500">{currentPage} / {filteredPokemon.totalPages}</span>
                    <button 
