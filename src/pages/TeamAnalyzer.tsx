...
export default function TeamAnalyzer() {
    ...
    if (!teamsLoading) {
      if (allTeams.length > 0) {
        ...
        const initialTeam: Team = {
          id: `team-${Date.now()}`,
          name: t('teamAnalyzer.defaultTeamName', { number: 1 }),
  ...
  const updateCurrentTeam = (members: TeamMember[]) => {
    if (activeTeamId) {
      updateTeam(activeTeamId, { members });
    }
  };
  ...
  const createNewTeam = () => {
    const newTeam: Team = {
      id: `team-${Date.now()}`,
      name: t('teamAnalyzer.defaultTeamName', { number: allTeams.length + 1 }),
      gameId: selectedGame.id,
      members: [],
      createdAt: Date.now()
    };
  ...
  const deleteTeam = (id: string) => {
    ...
    removeTeam(id);
    if (activeTeamId === id) {
      const remainingTeams = allTeams.filter(t => t.id !== id);
      if (remainingTeams.length > 0) {
        setActiveTeamId(remainingTeams[0].id);
        localStorage.setItem('rotomdex_last_active_team', remainingTeams[0].id);
      }
    }
  };

  const renameTeam = (id: string, newName: string) => {
    updateTeam(id, { name: newName });
  ...
  const handleGameChange = (game: Game) => {
    ...
    if (currentTeam && currentTeam.members.length === 0) {
      updateTeam(activeTeamId!, { gameId: game.id });
    }
  ...
  const generateOptimizedTeam = async (starterName: string) => {
    ...
    try {
      ...
      const prompt = `Sei un esperto di Pokémon. Genera una squadra ottimizzata di ESATTAMENTE 6 Pokémon per il gioco "${selectedGame.isCustom ? selectedGame.name : t(selectedGame.name)}" partendo dallo starter "${starterName}".
      
      IMPORTANTE: Usa SOLO Pokémon che sono CATTURABILI o OTTENIBILI in "${selectedGame.isCustom ? selectedGame.name : t(selectedGame.name)}". 
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      if (analysisMode === 'casual') {
        prompt = t('teamAnalyzer.casualPrompt', { game: selectedGame.isCustom ? selectedGame.name : t(selectedGame.name), team: teamInfo });
      } else {
        prompt = t('teamAnalyzer.competitivePrompt', { game: selectedGame.isCustom ? selectedGame.name : t(selectedGame.name), team: teamInfo });
      }
        ...
        <div className="flex flex-col w-full lg:flex-1 lg:min-w-0 gap-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            {editingTeamName !== null ? (
              ...
              <h2 className="text-3xl md:text-5xl lg:text-6xl font-black uppercase italic tracking-tighter flex items-center flex-wrap gap-3 break-words overflow-hidden">
                <span className="max-w-full truncate lg:whitespace-normal lg:overflow-visible lg:break-words">
                  {currentTeam?.name || t('teamAnalyzer.title')}
                </span>
                <button 
                  onClick={() => setEditingTeamName(activeTeamId!)}
                  className="p-2 hover:text-blue-500 transition-colors shrink-0"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <span className="text-xs font-black uppercase italic text-white truncate max-w-[80px] sm:max-w-none block">{selectedGame.isCustom ? `✨ ${selectedGame.name}` : t(selectedGame.name)}</span>
                  </div>
                </div>
                  ...
                  >
                    <div className="max-h-64 overflow-y-auto custom-scrollbar">
                      {availableGames.map(game => (
                        ...
                        >
                          <span className="text-[10px] font-black uppercase italic">{game.isCustom ? `✨ ${game.name}` : t(game.name)}</span>
