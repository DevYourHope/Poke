...
export default function ShinyHunting() {
  ...
  const [shinyLog, setShinyLog] = useState<ShinyRecord[]>([]);
  ...
  // Load custom games and shiny log
  useEffect(() => {
    ...
    const savedLog = localStorage.getItem('rotomdex_shiny_log') || localStorage.getItem('auradex_shiny_log');
    if (savedLog) {
      setShinyLog(JSON.parse(savedLog));
    }
  }, []);

  const saveShinyLog = (log: ShinyRecord[]) => {
    setShinyLog(log);
    localStorage.setItem('rotomdex_shiny_log', JSON.stringify(log));
  };
  ...
  useEffect(() => {
    const fetchInitialPokemon = async () => {
      try {
        // Fetch more to include regional forms
  ...
  const handleRegisterShiny = () => {
    if (!selectedPokemon) return;
    
    const newRecord: ShinyRecord = {
      id: `shiny-${Date.now()}`,
    ...
    saveShinyLog([newRecord, ...shinyLog]);
    
    setEncounters(0);
    setNotes('');
    alert(`✨ ${selectedPokemon.name} Shiny salvato nel Diario! ✨`);
  };

  const deleteShinyRecord = (id: string) => {
    if (confirm('Vuoi davvero eliminare questo record dal diario?')) {
      saveShinyLog(shinyLog.filter(r => r.id !== id));
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
        <div>
          <h2 className="text-4xl md:text-5xl font-black uppercase italic tracking-tighter">
            Shiny <span className="text-yellow-500">Hunter</span>
          </h2>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-xs mt-1">Modulo di tracciamento cromatico v2.0</p>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <div className="hidden md:flex items-center gap-3 px-6 py-3 bg-slate-900 rounded-2xl border border-slate-800">
            ...
            <span className="text-sm font-black uppercase italic">Sessione Attiva</span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <section className="bg-slate-900 rounded-[2rem] p-8 border border-slate-800 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Probabilità (1 / X)</label>
                <div className="relative">
                  <input 
                    type="number" 
                    value={customOdds}
                    onChange={(e) => setCustomOdds(Number(e.target.value))}
                    className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-3 text-lg font-black focus:ring-2 focus:ring-blue-500 outline-none transition-all pr-24"
                  />
                  <div className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-500 font-black italic text-[10px] uppercase tracking-tighter">Probabilità</div>
                </div>
                <p className="text-[10px] text-slate-500 mt-2 italic font-medium">
                  {selectedGame.isCustom ? `Versione personalizzata. Pokedex: ${selectedGame.pokedex}` : `Valore base per ${selectedGame.name}: 1/${selectedGame.baseOdds}`}
                </p>
              </div>

              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Note / Metodo di Cattura</label>
                <textarea 
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Es: Metodo Masuda, SOS Chaining, ecc."
          ...
          <section className="bg-slate-900 rounded-[2rem] p-8 border border-slate-800 shadow-2xl">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-slate-800/50 rounded-2xl border border-slate-800">
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Probabilità Singola</span>
                <div className="text-2xl font-black text-blue-400 italic">{baseProbPercent}%</div>
              </div>
              <div className="p-4 bg-slate-800/50 rounded-2xl border border-slate-800">
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Successo Cumulativo</span>
            ...
            {selectedPokemon ? (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-8 md:gap-16 mb-12">
                  <div className="flex flex-col items-center group">
                    <div className="w-40 h-40 md:w-56 md:h-56 bg-slate-900 rounded-full flex items-center justify-center border border-slate-800 relative overflow-hidden group-hover:border-slate-700 transition-all">
                      ...
                      <img src={selectedPokemon.sprite} alt={selectedPokemon.name} className="w-32 h-32 md:w-48 md:h-48 pixelated relative z-10 drop-shadow-2xl" />
                    </div>
                    <span className="mt-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Forma Base</span>
                  </div>
                  <div className="flex flex-col items-center group">
                    <div className="w-40 h-40 md:w-56 md:h-56 bg-slate-900 rounded-full flex items-center justify-center border border-yellow-500/30 relative overflow-hidden group-hover:border-yellow-500/50 transition-all shadow-[0_0_50px_rgba(234,179,8,0.1)]">
                      ...
                      <img src={selectedPokemon.shinySprite} alt={selectedPokemon.name} className="w-32 h-32 md:w-48 md:h-48 pixelated relative z-10 drop-shadow-[0_0_20px_rgba(234,179,8,0.5)]" />
                    </div>
                    <span className="mt-4 text-[10px] font-black text-yellow-500 uppercase tracking-widest flex items-center gap-1">
                      <Sparkles size={12} /> Forma Cromatica
                    </span>
                  </div>
                </div>
                ...
                <div className="w-full max-w-lg bg-slate-900 rounded-[3rem] p-10 border border-slate-800 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="text-center mb-10">
                    <span className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] mb-3 block">Incontri Totali</span>
                  ...
                  {/* Probability Stats Above Arrows */}
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-slate-800/50 rounded-2xl p-3 border border-slate-800 text-center">
                      <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Prob. Base</span>
                      <div className="text-sm font-black text-blue-400 italic">{baseProbPercent}%</div>
                    </div>
                    <div className="bg-slate-800/50 rounded-2xl p-3 border border-slate-800 text-center">
                      <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Prob. Totale</span>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-32">
                <motion.div 
                  ...
                  className="w-32 h-32 bg-slate-900 rounded-full flex items-center justify-center mb-8 border border-slate-800 shadow-2xl"
                >
                  <Search size={56} className="text-slate-700" />
                </motion.div>
                <h3 className="text-3xl font-black text-slate-400 uppercase italic tracking-tighter">Inizia la Caccia</h3>
                <p className="text-slate-600 max-w-xs mt-4 font-medium leading-relaxed">
                  Seleziona un Pokémon dal database per iniziare a tracciare i tuoi incontri.
                </p>
                <button 
                  onClick={() => setIsSelectorOpen(true)}
      ...
      <section className="mt-20">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-3xl font-black uppercase italic tracking-tighter flex items-center gap-3">
              ...
              Diario <span className="text-yellow-500">Cromatico</span>
            </h3>
            <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] mt-1">La tua bacheca dei trofei</p>
          </div>
        ...
        {shinyLog.length === 0 ? (
          <div className="py-20 text-center bg-slate-900/30 rounded-[3rem] border border-dashed border-slate-800">
            ...
            <p className="text-slate-600 font-bold uppercase text-xs tracking-widest">Nessun Pokémon cromatico registrato nel diario</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex items-center gap-6">
                    <div className="w-24 h-24 bg-slate-800/50 rounded-3xl flex items-center justify-center relative">
                      ...
                      <img src={record.pokemon.shinySprite} alt={record.pokemon.name} className="w-20 h-20 pixelated relative z-10 drop-shadow-[0_0_10px_rgba(234,179,8,0.3)]" />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest truncate">{record.game.name}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-6">
                    <div className="bg-slate-800/50 rounded-2xl p-3 border border-slate-800">
                      <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Incontri</span>
                      <div className="text-lg font-black text-white italic">{record.encounters}</div>
                    </div>
                    <div className="bg-slate-800/50 rounded-2xl p-3 border border-slate-800">
                      <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Data</span>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
            ...
            >
              <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 sticky top-0 z-10">
                <div>
                  <h3 className="text-3xl font-black uppercase italic tracking-tighter">Database Pokémon</h3>
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Seleziona il tuo obiettivo</p>
                </div>
                <button 
                  onClick={() => setIsSelectorOpen(false)}
              ...
              <div className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
                <div className="relative">
                  ...
                  <input 
                    type="text"
                    placeholder="Cerca per nome o numero Pokédex..."
                    value={searchTerm}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {isGameDexLoading ? (
                    <div className="col-span-full py-20 text-center text-slate-500 font-black uppercase italic tracking-widest">Sincronizzazione Pokédex Regionale...</div>
                  ) : isLoading ? (
                    <div className="col-span-full py-20 text-center text-slate-500 font-black uppercase italic tracking-widest">Caricamento Database...</div>
                  ) : filteredPokemon.length === 0 ? (
                    <div className="col-span-full py-20 text-center text-slate-500 font-black uppercase italic tracking-widest">Nessun Pokémon trovato</div>
